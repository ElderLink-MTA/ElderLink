...
// Fetch the request times for each request
$requests = [];
while ($row = $requests_result->fetch_assoc()) {
    $req_id = $row['req_id'];
    $times_sql = "SELECT * FROM requests_times WHERE req_id = '$req_id'";
...
<head>
  <title>
    ElderLink - בקשות ממתינות לשיבוץ
  </title>
  <meta charset="utf-8">
...
<body>
  ...
  <main class="d-flex flex-nowrap">
      ...
      <div class="container-xl px-4 mt-2">
        <nav class="nav nav-borders">
          <a class="nav-link" href="placed_requests.php">
            <span class="active-request-indicator"></span> בקשות פעילות (<?php echo $active_count; ?>)
          </a>
          <a class="nav-link active" href="waiting_requests.php">
            <span class="wait-request-indicator"></span> בקשות ממתינות לשיבוץ (<?php echo $waiting_requests_count; ?>)
          </a>
            ...
            <div class="wrapper w-100">
              <div>
                <div class="list-group pt-3 gap-2">
                  ...
                  <div class="list-group-item list-group-item-action d-flex W p-2" aria-current="true">
                    <img src="../../images/anonymous.png" alt="תמונת פרופיל" width="44" height="44"
                    ...
                    <div class="d-flex w-100 justify-content-between">
                      <div class="justify-content-between">
                        ...
                        <small class="opacity-75 text-nowrap text-decoration-underline">פרטי הבקשה</small>
                        ...
                        <small class="opacity-75 text-nowrap text-decoration-underline">חלונות זמנים אפשריים</small>
                        <?php foreach ($request['times'] as $index => $time) { ?>
                        <div class="gap-2" style="display: flex; flex-direction: row;">
                          ...
                          <p>&bull;</p>
                          <p><b>שעת התחלה: </b>
                            <?php echo date("H:i", strtotime($time['start_time'])); ?>
                          </p>
                          <p>&bull;</p>
                          <p><b>שעת סיום: </b>
                            <?php echo date("H:i", strtotime($time['end_time'])); ?>
                          </p>
                        </div>
                        <?php } ?>
                      </div>
                      <div class="d-flex">
                        <div class="col-md-6 text-center" style="margin-left:5px;">
                          <a href="update_request.php?
                                                    req_id=<?php echo $request['req_id']; ?>&
                                                    city=<?php echo urlencode($request['city']); ?>&
                                                    street=<?php echo urlencode($request['street']); ?>&
                                                    street_num=<?php echo urlencode($request['street_num']); ?>&
                                                    req_type=<?php echo urlencode($request['req_type']); ?>&
                                                    content=<?php echo urlencode($request['content']); ?>&
                                                    times=<?php echo urlencode(json_encode($request['times'])); ?>"
                            class="btn btn-sm btn-success">עדכון</a>
                        </div>
                        <div class="col-md-6 text-center">
                          <button class="btn btn-sm btn-danger"
                            onclick="cancelRequest(<?php echo $request['req_id']; ?>)">ביטול</button>
                        </div>
                      </div>
                    </div>
                  </div>
                  <?php } 
                      if ($waiting_requests_count == 0) {
                                                         echo '<div class="list-group pt-3 gap-2">
                                        <div style="            display: flex;
                justify-content: center;
                align-items: center;">
                                            <img src="../../images/time.png" alt="Cart" width="350" height="350">
                                        </div>
                                        <h4 style="text-align:center; font-weight: bold; ">עדיין לא פתחתם בקשה?</h4>
                                        <p style="text-align:center;">עדיין אין לכם בקשות ממתינות לשיבוץ. ברגע שתפתחו בקשה, היא תועבר לכאן.</p>
                                </div>';
                      }
                  ?>
                </div>
    ...
    <script src="../../js/waiting_requests.js"></script>
